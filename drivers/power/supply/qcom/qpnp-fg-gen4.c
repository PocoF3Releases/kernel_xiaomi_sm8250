					...
					struct device_node *profile_node)
{
	...
	/* Changed only when parsing a selected profile, never by FFC. */
	WRITE_ONCE(chip->profile_max_voltage_uv, fg->bp.float_volt_uv);
...
/* The selected profile limit must not become an FFC runtime setpoint. */
static int fg_gen4_get_profile_voltage(struct fg_gen4_chip *chip, int *uv)
{
	int limit = READ_ONCE(chip->profile_max_voltage_uv);
...
/* Preserve the existing 10 mV CC/CV-to-float headroom for replacements. */
static int fg_gen4_replacement_cc_cv_mv(int requested_mv, int profile_uv)
{
	if (requested_mv <= 0 || requested_mv > INT_MAX / 1000 - 10 ||
	    profile_uv < 3500000 || profile_uv > 5000000)
		return -EINVAL;

	return min(requested_mv, profile_uv / 1000 - 10);
}

static int fg_gen4_set_vbatt_full_vol(struct fg_dev *fg, bool enable_ffc)
{
	struct fg_gen4_chip *chip = container_of(fg, struct fg_gen4_chip, fg);
	int rc, volt, runtime_fv_uv, profile_uv = 0;

	volt = enable_ffc ? fg->bp.ffc_vbatt_full_mv : fg->bp.vbatt_full_mv;
	if (volt <= 0 || volt > INT_MAX / 1000 - 10)
		return -EINVAL;

	if (READ_ONCE(chip->replacement_profile_fallback)) {
		rc = fg_gen4_get_profile_voltage(chip, &profile_uv);
		if (rc < 0)
			return rc;
		volt = fg_gen4_replacement_cc_cv_mv(volt, profile_uv);
		if (volt < 0)
			return volt;
	}
	...
	/* Keep the legacy runtime calibration for positively identified packs. */
	runtime_fv_uv = volt * 1000 + 10000;
	if (profile_uv > 0)
		runtime_fv_uv = min(runtime_fv_uv, profile_uv);
	WRITE_ONCE(fg->bp.float_volt_uv, runtime_fv_uv);
				       ...
				       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX_DESIGN:
		if (fg->battery_missing || !fg->profile_available ||
		    !fg->soc_reporting_ready)
			rc = -ENODATA;
		else
			rc = fg_gen4_get_profile_voltage(chip, &pval->intval);
		break;
